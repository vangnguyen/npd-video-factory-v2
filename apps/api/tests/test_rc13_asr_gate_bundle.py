...
BUNDLE_PATH = ACCEPTANCE_ROOT / "V3-01-GATE-RC13-OPENAI-ASR-A.json"
EXPECTED_RC = "1e0146b44b19a5afcef267132d71d36d24a952e4"
EXPECTED_BUNDLE_SHA = "236262caf3ae4a10c8c3fa760e9caf134837e4327b860b2d4693e08c7031f1b8"
EXPECTED_SCOPE_SHA = "179624fe3a365e415c41b760e49297da0cd23227cf9b49a634e7fbcaaf90b47e"
ACTIVE_AT = datetime(2026, 9, 6, 14, 30, tzinfo=timezone.utc)
...
def _active_policy():
    settings = Settings(
        ...
        provider_gate_expected_rc_tag="vf-v3-01-rc13",
...
def _asr_context(slot: int, **updates: object) -> ProviderCallContext:
    ...
    values: dict[str, object] = {
        ...
        "workspace_id": "wsp_rc13_asr_acceptance",
        "project_id": "prj_rc13_asr_acceptance",
...
def test_rc13_asr_bundle_loads_with_fresh_exact_bindings() -> None:
    ...
    assert [item.operation_key for item in scope.allowed_operations] == [
        "v3-01-rc13-openai-transcription-asr-call-01",
        "v3-01-rc13-openai-transcription-asr-call-02",
    ]
    assert all("rc12" not in item.operation_key for item in scope.allowed_operations)


def test_rc13_asr_bundle_embeds_exact_rebind_records() -> None:
    payload = json.loads(BUNDLE_PATH.read_text(encoding="utf-8"))
    bundle = OpenAIAsrGateBundle.model_validate(payload)

    for key, filename in (
        ("credential_approval", "V3-01-APP-053.json"),
        ("budget_approval", "V3-01-APP-054.json"),
        ("rights_approval", "V3-01-APP-055.json"),
    ):
...
def test_rc13_rebind_preserves_exact_assets_transcripts_and_rights() -> None:
...
def test_rc13_rebind_keeps_checked_in_runtime_fail_closed() -> None:
...
async def test_rc13_durable_and_non_durable_select_same_exact_rights(
    ...
    durable, operation_count = await _durable_preflight(
        tmp_path, context, f"rc13-slot-{slot}"
    )
...
@pytest.mark.asyncio
async def test_rc13_wrong_asset_fails_before_durable_operation_row(tmp_path: Path) -> None:
    ...
    durable, operation_count = await _durable_preflight(
        tmp_path, context, "rc13-wrong-asset"
    )
