...
BUNDLE_PATH = ACCEPTANCE_ROOT / "V3-01-GATE-RC17-OPENAI-ASR-W1-LINEAGE-A.json"
EXPECTED_RC = "d08ffc005d7f3ad517d355977b0bc3cc8d686906"
EXPECTED_BUNDLE_SHA = "39867efb2a95d22bf5d4be64e041671cf10517a118d02bece010778ab587a76f"
EXPECTED_SCOPE_SHA = "6b1d5f25684d0b276c06c4b80b5d636f7845a36fdedc24e88adbd5bae7930fc7"
EXPECTED_PROFILE_SHA = "9c4a7609db9f08c191af297a41d7a21b58bfe539ac5e100ea534196d17776ab1"
EXPECTED_LINEAGE = "al-0001-b82fc34d364bd33a73c9f7eb9d99a8108d77be25449ab5e35a4df7f1ec380329"
ACTIVE_AT = datetime(2026, 9, 12, 14, 30, tzinfo=timezone.utc)
...
def _active_policy():
    settings = Settings(
        ...
        provider_gate_expected_rc_tag="vf-v3-01-rc17",
        provider_gate_expected_acceptance_lineage_id=EXPECTED_LINEAGE,
...
def _asr_context(slot: int, **updates: object) -> ProviderCallContext:
    ...
    values: dict[str, object] = {
        ...
        "acceptance_lineage_id": EXPECTED_LINEAGE,
        "workspace_id": "wsp_rc17_asr_w1_lineage_acceptance",
        "project_id": "prj_rc17_asr_w1_lineage_acceptance",
...
def test_rc17_v2_bundle_loads_with_exact_lineage_profile_and_scope() -> None:
    ...
    scope = load_verified_provider_gate_bundle(
        ...
        expected_rc_tag="vf-v3-01-rc17",
        expected_acceptance_lineage_id=EXPECTED_LINEAGE,
    )
    assert scope.gate_bundle_version == 2
    assert scope.acceptance_lineage_sequence == 1
    assert scope.acceptance_lineage_id == EXPECTED_LINEAGE
...
def test_rc17_lineage_is_canonical_for_exact_execution_identity() -> None:
    assert EXPECTED_LINEAGE == derive_acceptance_lineage_id(
        rc_tag="vf-v3-01-rc17",
        rc_commit=EXPECTED_RC,
        provider_key="openai-transcription",
        model="whisper-1",
        capability="asr",
        sequence=1,
...
def test_rc17_bundle_embeds_exact_rebind_records_and_rights() -> None:
    bundle = OpenAIAsrGateBundle.model_validate_json(BUNDLE_PATH.read_text(encoding="utf-8"))
    for key, filename in (
        ("credential_approval", "V3-01-APP-069.json"),
        ("budget_approval", "V3-01-APP-070.json"),
        ("rights_approval", "V3-01-APP-071.json"),
    ):
...
def test_rc17_rebind_preserves_exact_assets_references_and_audio_contract() -> None:
...
def test_rc17_checked_in_runtime_remains_fail_closed() -> None:
    ...
    assert settings.provider_gate_expected_acceptance_lineage_id == ""
...
async def test_rc17_same_lineage_profile_and_rights_for_both_slots(
    ...
    durable, operation_count = await _durable_preflight(
        tmp_path, context, f"rc17-lineage-slot-{slot}"
    )
    ...
    assert context.acceptance_lineage_id == EXPECTED_LINEAGE


@pytest.mark.asyncio
@pytest.mark.parametrize("lineage", [None, "al-0001-" + "0" * 64])
async def test_rc17_missing_or_tampered_context_lineage_fails_before_durable_row(
    tmp_path: Path, lineage: str | None
) -> None:
    context = _asr_context(1, acceptance_lineage_id=lineage)
    durable, operation_count = await _durable_preflight(
        tmp_path, context, "rc17-context-lineage-denied"
    )
    assert durable.allowed is False
    assert durable.code == "ACCEPTANCE_LINEAGE_MISMATCH"
    assert operation_count == 0


@pytest.mark.asyncio
async def test_rc17_old_lineage_operation_cannot_be_reused(tmp_path: Path) -> None:
    context = _asr_context(
        1,
        operation_key="v3-01-rc16-openai-transcription-asr-call-01",
    )
    durable, operation_count = await _durable_preflight(
        tmp_path, context, "rc17-old-operation-denied"
    )
    assert durable.allowed is False
    assert durable.code == "OPERATION_NOT_ALLOWLISTED"
    assert operation_count == 0


@pytest.mark.parametrize("mutation", ["missing", "tampered", "sequence", "operation"])
def test_rc17_bundle_rejects_invalid_or_stale_lineage_binding(
    tmp_path: Path, mutation: str
) -> None:
    payload = json.loads(BUNDLE_PATH.read_text(encoding="utf-8"))
    if mutation == "missing":
        del payload["acceptance_lineage_id"]
    elif mutation == "tampered":
        payload["acceptance_lineage_id"] = "al-0001-" + "0" * 64
    ...
    path = tmp_path / f"rc17-{mutation}.json"
    path.write_text(json.dumps(payload), encoding="utf-8")
    with pytest.raises((ValidationError, ValueError)):
        OpenAIAsrGateBundle.model_validate_json(path.read_text(encoding="utf-8"))


def test_rc17_v2_bundle_requires_explicit_runtime_lineage_pin() -> None:
...
def test_rc17_scope_is_not_operation_authority() -> None:
    bundle = OpenAIAsrGateBundle.model_validate_json(BUNDLE_PATH.read_text(encoding="utf-8"))
    assert bundle.valid_from_utc == datetime(2026, 9, 12, 14, tzinfo=timezone.utc)
    assert bundle.expires_at_utc == datetime(2026, 9, 12, 18, tzinfo=timezone.utc)
    ...
    for approval in (
        ...
        assert "not execution authority" in approval.record.notes.lower()
