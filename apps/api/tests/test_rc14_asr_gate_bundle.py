...
BUNDLE_PATH = ACCEPTANCE_ROOT / "V3-01-GATE-RC14-OPENAI-ASR-A.json"
EXPECTED_RC = "0b0965c650f4d06a057acbbb1a7ed9d7b933478b"
EXPECTED_BUNDLE_SHA = "351f1cac3dd90df7ffc8a0b8b70e3c3c9a92480b91d258b012426f570cc410dd"
EXPECTED_SCOPE_SHA = "03c2ad3bf2173a890344db0b011ab932fac35a96e98df0f8e501ba7261b7b852"
ACTIVE_AT = datetime(2026, 9, 7, 14, 30, tzinfo=timezone.utc)
...
def _active_policy():
    settings = Settings(
        ...
        provider_gate_expected_rc_tag="vf-v3-01-rc14",
...
def _asr_context(slot: int, **updates: object) -> ProviderCallContext:
    ...
    values: dict[str, object] = {
        ...
        "workspace_id": "wsp_rc14_asr_acceptance",
        "project_id": "prj_rc14_asr_acceptance",
...
def test_rc14_asr_bundle_loads_with_fresh_exact_bindings() -> None:
    ...
    assert [item.operation_key for item in scope.allowed_operations] == [
        "v3-01-rc14-openai-transcription-asr-call-01",
        "v3-01-rc14-openai-transcription-asr-call-02",
    ]
    assert all(
        "rc13" not in item.operation_key and "rc12" not in item.operation_key
        for item in scope.allowed_operations
    )


def test_rc14_asr_bundle_embeds_exact_rebind_records() -> None:
    payload = json.loads(BUNDLE_PATH.read_text(encoding="utf-8"))
    bundle = OpenAIAsrGateBundle.model_validate(payload)

    for key, filename in (
        ("credential_approval", "V3-01-APP-057.json"),
        ("budget_approval", "V3-01-APP-058.json"),
        ("rights_approval", "V3-01-APP-059.json"),
    ):
...
def test_rc14_rebind_preserves_exact_assets_transcripts_and_rights() -> None:
...
def test_rc14_rebind_keeps_checked_in_runtime_fail_closed() -> None:
...
async def test_rc14_durable_and_non_durable_select_same_exact_rights(
    ...
    durable, operation_count = await _durable_preflight(
        tmp_path, context, f"rc14-slot-{slot}"
    )
...
@pytest.mark.asyncio
async def test_rc14_wrong_asset_fails_before_durable_operation_row(tmp_path: Path) -> None:
    ...
    durable, operation_count = await _durable_preflight(
        tmp_path, context, "rc14-wrong-asset"
    )
